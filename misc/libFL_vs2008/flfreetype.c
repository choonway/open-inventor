...
_flFTGetBitmap(FLfontStruct *_fs, GLuint c)
{
  ...
  if (glyph->format == ft_glyph_format_outline) {    
	...
	bit3->xorig = BM_TRUNC(-glyph->metrics.horiBearingX);
	bit3->yorig = BM_TRUNC(glyph->metrics.height -glyph->metrics.horiBearingY);

	//printf("'%c'",c);
	//printf("advance.x %d\n", BM_TRUNC(glyph->advance.x));

    //bit3->xmove     = width > 0 ? width : (bbox_width / 2.0);
	//bit3->ymove     = 0.0;

	bit3->xmove = BM_TRUNC(glyph->advance.x);
	bit3->ymove = BM_TRUNC(glyph->advance.y);
    ...
    //bit3->xorig     = bearing_x;
    //bit3->yorig     = height - bearing_y;
	bit3->xorig = BM_TRUNC(0);
	bit3->yorig = BM_TRUNC(0);
