		...
		unsigned int xsize, unsigned int ysize, unsigned int zsize)
{
	...
	if (*mode=='w') {
		...
		image->wastebytes = 0;
		image->dorev = 0;
		if (_write(f,image,sizeof(IMAGE)) != sizeof(IMAGE)) {
		    free(image);
		    _close(f);
		...
		if (_read(f,image,sizeof(IMAGE)) != sizeof(IMAGE)) {
		    free(image);
		    _close(f);
	...
	if(ISRLE(image->type)) {
		...
		tablesize = image->ysize*image->zsize*sizeof(int);
		_lseek(f, 512L, 0);
		if (_read(f,image->rowstart,tablesize) != tablesize) {
		    free(image);
		    _close(f);
		...
		if (_read(f,image->rowsize,tablesize) != tablesize) {
		    free(image);
		    _close(f);
