...
class INVENTOR_API SoDataSensor : public SoDelayQueueSensor {
  ...
  public:
    ...
    // The following three functions are only useful if
    // getTriggerNode() is an SoGroup and 
	// getTriggerType()!=SoGroup::GROUP_UNSPECIFIED
	// Returns the child which caused the trigger
	SoNode *        getTriggerChild() const;

	// Returns the index of the child which caused the trigger.
	// Only useful if getTriggerType is GROUP_REPLACECHILD and 
	// GROUP_INSERTCHILD or GROUP_REMOVECHILD
	int             getTriggerIndex() const;

    // Returns the type of the action which caused this trigger event.
	// See SoGroup.h for enum types. 
	int             getTriggerType() const;

    // The following two functions are only useful if
    // getTriggerNode() is an SoSF(M)Node
	// Returns the node which caused the trigger
	SoNode *        getTriggerPrevNode() const;
    
	// Returns the field which caused the trigger
    SoField *       getTriggerPrevField() const;
  ...
  private:
    ...
    int         trigChangeType; // change type (if parent is SoGroup)
	int         trigIndex; // index of changed child in group (if parent is SoGroup)
    SoField     *trigPreviousField;  // Field that triggered sensor (if node == SoSFNode||SoMFNode)
	SoNode      *trigPreviousNode;   // Node that triggered sensor (if node == SoSFNode||SoMFNode)
};
