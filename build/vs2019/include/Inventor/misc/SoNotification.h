...
SoINTERNAL class SoNotList {
  ...
  public:
    ...
    // Returns previous field set by notification (or NULL if notification
    // did not originate at or propagate through an SoSF(M)Node)
	SoField *       getPreviousField() const    { return previousField; }

    // Returns previous record set by notification (or NULL if notification
    // did not originate at or propagate through an SoSF(M)Node)
	SoNotRec *        getPreviousNodeRec() const     { return previousNodeRec; }
  ...
  private:
	...
	SoField         *previousField; // Previous field that changed
	SoNotRec        *previousNodeRec; // Node of previous field
